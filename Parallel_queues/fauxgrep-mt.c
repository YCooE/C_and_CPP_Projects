...
int main(int argc, char * const *argv) {
  ...
  while ((p = fts_read(ftsp)) != NULL) {
    ...
    case FTS_D:
      break;
    case FTS_F:
      job_queue_push(&wa.jq, (void*)strdup(p->fts_path)); // Process the file p->fts_path, somehow.
      break;
    default:
      break;
    }
  ...
  job_queue_destroy(&wa.jq); // Shut down the job queue and the worker threads here.
